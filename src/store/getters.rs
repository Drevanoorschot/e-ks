use crate::{AppError, candidate_lists::{CandidateList, CandidateListId}, persons::{Person, PersonId}, political_groups::{AuthorisedAgent, AuthorisedAgentId, ListSubmitter, ListSubmitterId, PoliticalGroup}};

use super::AppStore;

impl AppStore {
    pub fn get_candidate_lists(&self) -> Result<Vec<CandidateList>, AppError> {
        let data = self.data.read().map_err(|_| AppError::InternalServerError)?;
