...
export class CorrespondenceAddressPage {
  ...
  async setCorrespondenceAddress(candidate: Candidate) {
    ...
    await this.textfieldHouseNumber.pressSequentially(candidate.houseNumber ?? "");
    await this.textfieldHouseNumberAddition.pressSequentially(candidate.houseNumberAddition ?? "");
    //await this.page.locator("body").click();
    await expect(this.textfieldStreetName).toHaveValue(candidate.streetName ?? "");
